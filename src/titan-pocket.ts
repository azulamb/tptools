interface TitanPocketElement extends HTMLElement
{
	addEventListener( type: 'push', listener: ( event: ButtonPushEvent ) => any, options?: boolean | AddEventListenerOptions ): void;
}

type TITAN_POCKET_BUTTONS = 'volup' | 'voldown' | 'ptt' | 'power';

interface ButtonPushData
{
	button: TITAN_POCKET_BUTTONS;
}

interface ButtonPushEvent extends CustomEvent
{
	detail: ButtonPushData;
}
...
} )( <HTMLScriptElement>document.currentScript, ( script: HTMLScriptElement ) =>
{
	...
	} )( class extends HTMLElement implements TitanPocketElement
	{
		...
		constructor()
		{
			...
			[ 'volup', 'voldown', 'ptt', 'power' ].forEach( ( name: TITAN_POCKET_BUTTONS ) =>
			{
				const button = document.createElement( 'button' );
				button.classList.add( name );
				button.addEventListener( 'click', () =>
				{
					this.dispatchEvent( new CustomEvent<ButtonPushData>( 'push', { detail: { button: name } } ) );
				} );
